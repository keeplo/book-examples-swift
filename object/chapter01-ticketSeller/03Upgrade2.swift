...
private class FirstUpgrade {
    // MARK: - 설계 개선하기 2. buy() 추가 -> Audience 자율적 존재 만들기
    struct Invitation {
    ...
    class TicketSeller {
        ...
        func sellTo(audience: Audience) {
            guard let leftTicket = ticketOffice.getTicket() else {
                print("남은 티켓이 없습니다.")
                return
            }
    
            let ticketProfit = audience.buy(leftTicket)
            ticketOffice.plushAmout(ticketProfit)
        }
    }
